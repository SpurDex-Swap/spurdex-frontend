...
import { Text, Flex, Box, Skeleton, TooltipText, useTooltip } from '@spurdex/uikit'
...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  const isCurrencyCake = ifo.currency === tokens.spdx
  ...
  const basicTooltipContent =
    ifo.version === 3.1
      ? t(
          'For the private sale, each eligible participant will be able to commit any amount of SPDX up to the maximum commit limit, which is published along with the IFO voting proposal.',
        )
      : t(
          'For the basic sale, Max SPDX entry is capped by minimum between your average SPDX balance in the IFO SPDX pool, or the pool’s hard cap. To increase the max entry, Stake more SPDX into the IFO SPDX pool',
        )

  const unlimitedToolipContent =
    ifo.version === 3.1
      ? t(
          'For the public sale, Max SPDX entry is capped by your average SPDX balance in the IFO SPDX pool. To increase the max entry, Stake more SPDX into the IFO SPDX pool',
        )
      : t(
          'For the unlimited sale, Max SPDX entry is capped by your average SPDX balance in the IFO SPDX pool. To increase the max entry, Stake more SPDX into the IFO SPDX pool',
        )

  const tooltipContent = poolId === PoolIds.poolBasic ? basicTooltipContent : unlimitedToolipContent

  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })
  const label = isCurrencyCake ? t('Max. SPDX entry') : t('Max. token entry')
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {ifo[poolId].cakeToBurn !== '$0' && <FooterEntry label={t('SPDX to burn:')} value={ifo[poolId].cakeToBurn} />}
          <FooterEntry
