...
import { Text, Flex, AccountIcon, TeamBattleIcon, Box, useTooltip, LinkExternal } from '@spurdex/uikit'
...
const NotOkNFT = ({ admissionProfile }) => {
  ...
  const keyword = '%Spurdex Squad NFT%'

  const rawText = t(`Set %Spurdex Squad NFT% as Spurdex Profile avatar`)
      ...
      element={
        <>
          <LinkExternal
            ...
            href={`https://spurdex.exchange/nfts/collections/${admissionProfile}`}
          >
            {t('Spurdex Squad NFT')}
          </LinkExternal>
          <br />
...
const NotOkProfilePoints = ({ pointThreshold }) => {
  ...
  const keyword = '%Spurdex Profile%'

  const rawText = t(`Reach %point% or more %Spurdex Profile% points`, { point: pointThreshold })
    ...
    <TransWithElement
      ...
      element={
        <>
          ...
          <LinkExternal
            style={{ display: 'inline' }}
            href={`https://spurdex.exchange/nfts/profile/${account}/achievements/`}
          >
            {t('Spurdex Profile')}
          </LinkExternal>
        </>
