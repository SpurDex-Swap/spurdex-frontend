...
import { Box, Text, Flex, MessageText, Message } from '@spurdex/uikit'
  ...
  return (
    <>
      <Box>
        ...
        <LightGreyCard>
          <BalanceRow title={t('Spdx to be locked')} value={lockedAmount} newValue={newLockedAmount} decimals={2} />
          <BalanceRow
            title="apy"
            unit="%"
      ...
      {showLockWarning && (
        <Box mt="16px" maxWidth="370px">
          <Message variant="warning">
            <MessageText>
              {t('You will be able to withdraw the staked SPDX and profit only when the staking position is unlocked')}
            </MessageText>
          </Message>
        </Box>
      )}
