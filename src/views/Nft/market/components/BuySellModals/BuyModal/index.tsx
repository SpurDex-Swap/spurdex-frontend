...
import { InjectedModalProps } from '@spurdex/uikit'
...
import { ChainId } from '@danielvindax/spurdexswap-sdk'
...
const wbnbAddress =
  CHAIN_ID === String(ChainId.MAINNET) ? tokens.wcredit.address : '0x094616f0bdfb0b526bd735bf66eca0ad254ca81f'

const BuyModal: React.FC<BuyModalProps> = ({ nftToBuy, onDismiss }) => {
  const [stage, setStage] = useState(BuyingStage.REVIEW)
  const [confirmedTxHash, setConfirmedTxHash] = useState('')
  const [paymentCurrency, setPaymentCurrency] = useState<PaymentCurrency>(PaymentCurrency.CREDIT)
  ...
  const walletBalance = paymentCurrency === PaymentCurrency.CREDIT ? formattedBnbBalance : formattedWbnbBalance
  const walletFetchStatus = paymentCurrency === PaymentCurrency.CREDIT ? bnbFetchStatus : wbnbFetchStatus

  const notEnoughBnbForPurchase =
    paymentCurrency === PaymentCurrency.CREDIT
  ...
  useEffect(() => {
    if (bnbBalance.lt(nftPriceWei) && wbnbBalance.gte(ethersToBigNumber(nftPriceWei)) && !isPaymentCurrentInitialized) {
      setPaymentCurrency(PaymentCurrency.WCREDIT)
  ...
  const continueToNextStage = () => {
    if (paymentCurrency === PaymentCurrency.WCREDIT && !isApproved) {
