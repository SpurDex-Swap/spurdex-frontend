...
import { Text, Flex, Skeleton, Image } from '@spurdex/uikit'
  ...
  useEffect(() => {
    const fetchBurnedCakeAmount = async () => {
      try {
        ...
        console.error('Failed to fetch burned auction spdx', error)
      }
    }
  ...
  return (
    <Flex flexDirection={['column-reverse', null, 'row']}>
      <Flex flexDirection="column" flex="2" ref={observerRef}>
        {burnedCakeAmount !== 0 ? (
          <Balance fontSize="64px" bold value={burnedCakeAmount} decimals={0} unit=" SPDX" />
        ) : (
      ...
      <Image width={350} height={320} src="/images/burnt-spdx.png" alt={t('Burnt SPDX')} />
    </Flex>
  )
}
