...
import { Currency, currencyEquals, ETHER, Percent, WETH } from '@danielvindax/spurdexswap-sdk'
import {
  Button,
  Text,
  AddIcon,
                  ...
                  {chainId && (oneCurrencyIsWBNB || oneCurrencyIsBNB) ? (
                    <RowBetween style={{ justifyContent: 'flex-end', fontSize: '14px' }}>
                      {oneCurrencyIsBNB ? (
                        ...
                        <StyledInternalLink
                          href={`/remove/${
                            currencyA && currencyEquals(currencyA, WETH[chainId]) ? 'CREDIT' : currencyIdA
                          }/${currencyB && currencyEquals(currencyB, WETH[chainId]) ? 'CREDIT' : currencyIdB}`}
                        >
                          {t('Receive CREDIT')}
                        </StyledInternalLink>
                      ) : null}
