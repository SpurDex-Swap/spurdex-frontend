import { Flex, Heading, Skeleton, Text } from '@spurdex/uikit'
 ...
 * https://planetfinanceio.medium.com/spurdex-works-with-planet-to-help-cake-holders-f0d253b435af
 * https://twitter.com/spurdex/status/1523913527626702849
...
const CakeDataRow = () => {
  ...
  const {
    ...
    async () => {
      const totalSupplyCall = { address: tokens.spdx.address, name: 'totalSupply' }
      const burnedTokenCall = {
        address: tokens.spdx.address,
        name: 'balanceOf',
  ...
  return (
    <Grid>
      ...
      <StyledColumn noDesktopBorder style={{ gridArea: 'a' }}>
        <Text color="#EFCD72" mb="10px">{t('Total supply')}</Text>
        {cakeSupply ? (
      ...
      <StyledColumn style={{ gridArea: 'b' }}>
        <Text color="#EFCD72" mb="10px">{t('Market cap')}</Text>
        {mcap?.gt(0) && mcapString ? (
          <Text fontSize="24px">{t('$%marketCap%', { marketCap: mcapString })}</Text>
        ) : (
          <Skeleton height={24} width={126} my="4px" />
        )}
      </StyledColumn>
      <StyledColumn style={{ gridArea: 'c' }}>
        <Text color="#EFCD72" mb="10px">{t('Burned to date')}</Text>
        {burnedBalance ? (
      ...
      <StyledColumn style={{ gridArea: 'd' }}>
        <Text color="#EFCD72" mb="10px">{t('Current emissions')}</Text>

        <Text fontSize="24px">{t('%cakeEmissions%/block', { cakeEmissions: emissionsPerBlock })}</Text>
      </StyledColumn>
    </Grid>
  )
}
