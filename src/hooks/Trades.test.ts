...
import { Pair, TokenAmount, CurrencyAmount, Trade } from '@danielvindax/spurdexswap-sdk'
...
describe('Trade', () => {
  describe('#useAllCommonPairs', () => {
    ...
    it('should filter only exist Pair', () => {
      mockUsePairs.mockReturnValue([
        [
          PairState.EXISTS,
          new Pair(new TokenAmount(mainnetTokens.wcredit, '1'), new TokenAmount(mainnetTokens.spdx, '1')),
        ],
        [
          PairState.INVALID,
          new Pair(new TokenAmount(mainnetTokens.cusd, '1'), new TokenAmount(mainnetTokens.spdx, '1')),
        ],
        [
          PairState.LOADING,
          new Pair(new TokenAmount(mainnetTokens.cusd, '1'), new TokenAmount(mainnetTokens.wcredit, '1')),
        ],
        [PairState.EXISTS, null],
      ])

      const { result } = renderHook(() => {
        const pairs = Trades.useAllCommonPairs(mainnetTokens.wcredit, mainnetTokens.spdx)
        return {
          pairs,
        }
      })
    ...
    it('should filter out duplicated Pair', () => {
      mockUsePairs.mockReturnValue([
        ...
        [
          PairState.EXISTS,
          new Pair(new TokenAmount(mainnetTokens.spdx, '1'), new TokenAmount(mainnetTokens.wcredit, '1')),
        ],
        [PairState.EXISTS, null],
      ])
    ...
    it('should get all pair combinations wcredit, spdx', () => {
      mockUsePairs.mockClear()
      renderHook(() => {
        Trades.useAllCommonPairs(mainnetTokens.wcredit, mainnetTokens.spdx)
      })

      expect(mockUsePairs).toMatchSnapshot()
    })

    it('should get all pair combinations, wcredit, wcredit', () => {
      mockUsePairs.mockClear()
      renderHook(() => {
        Trades.useAllCommonPairs(mainnetTokens.wcredit, mainnetTokens.wcredit)
      })
  ...
  describe('#useTradeExactIn/Out', () => {
    ...
    it('should call with maxHops 1 with singleHopOnly', () => {
      const allowPairs = [new Pair(new TokenAmount(mainnetTokens.wcredit, '1'), new TokenAmount(mainnetTokens.spdx, '1'))]
      const argA = CurrencyAmount.ether('1000000')
      const argB = mainnetTokens.spdx
      renderHook(
        () => {
