import { Currency, currencyEquals, ETHER, WETH } from '@danielvindax/spurdexswap-sdk'
      ...
      return {
        ...
        execute:
          sufficientBalance && inputAmount
            ? async () => {
                try {
                  ...
                  addTransaction(txReceipt, {
                    summary: `Wrap ${inputAmount.toSignificant(6)} CREDIT to WCREDIT`,
                    type: 'wrap',
                  })
                } catch (error) {
            ...
            : undefined,
        inputError: sufficientBalance ? undefined : t('Insufficient CREDIT balance'),
      }
    }
          ...
          sufficientBalance && inputAmount
            ? async () => {
                try {
                  ...
                  addTransaction(txReceipt, { summary: `Unwrap ${inputAmount.toSignificant(6)} WCREDIT to CREDIT` })
                } catch (error) {
                  console.error('Could not withdraw', error)
                }
              }
            : undefined,
        inputError: sufficientBalance ? undefined : t('Insufficient WCREDIT balance'),
      }
    }
