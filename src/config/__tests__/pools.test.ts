...
describe('Config pools', () => {
  ...
  it.each(pools.map((pool) => [pool.sousId, pool.contractAddress]))(
    ...
    (sousId, contractAddress) => {
      const duplicates = pools.filter((p) => contractAddress[13308] === p.contractAddress[13308])
      expect(duplicates).toHaveLength(1)
    },
  )
  it.each(poolsToTest.filter((pool) => pool.earningToken.symbol !== 'CREDIT'))(
  ...
  it.each(poolsToTest.filter((pool) => pool.stakingToken.symbol !== 'CREDIT'))(
