...
import { Currency, CurrencyAmount, TokenAmount, Trade, Token, Price, ETHER } from '@danielvindax/spurdexswap-sdk'
  ...
  const onCurrencySelection = useCallback(
    (field: Field, currency: Currency) => {
      dispatch(
        selectCurrency({
          field,
          currencyId: currency instanceof Token ? currency.address : currency === ETHER ? 'CREDIT' : '',
        }),
      )
    },
    [dispatch],
...
function parseCurrencyFromURLParameter(urlParam: any): string {
  if (typeof urlParam === 'string') {
    ...
    if (urlParam.toUpperCase() === 'CREDIT') return 'CREDIT'
    if (valid === false) return 'CREDIT'
  }
  return ''
}
