import { FACTORY_ADDRESS } from '@danielvindax/spurdexswap-sdk'
  ...
  const usersQuery = gql`
    query userCount($since: ISO8601DateTime, $till: ISO8601DateTime) {
      ethereum(network: bsc) {
        dexTrades(exchangeName: { in: ["Spurdex", "Spurdex v2"] }, date: { since: $since, till: $till }) {
          count(uniq: senders)
        }
      }
    }
  ...
  try {
    const result = await infoServerClient.request(gql`
      query tvl {
        ...
        token(id: "0x0C609476442DC803721BD8F697609050CBbe814E") {
          derivedUSD
        }
      }
    `)
