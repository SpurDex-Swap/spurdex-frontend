...
export const SiwtchUnits: React.FC = () => {
  const CAKE_PRICE = 69;
  const [editingUnit, setEditingUnit] = useState<"SPDX" | "USD">("SPDX");
  const conversionUnit = editingUnit === "SPDX" ? "USD" : "SPDX";
  ...
  const switchEditingUnits = () => {
    const editingUnitAfterChange = editingUnit === "SPDX" ? "USD" : "SPDX";
  ...
  const handleCakeChange = (input: string) => {
    ...
    if (editingUnit === "SPDX") {
      ...
      setValues({
        SPDX: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat / CAKE_PRICE}`,
        USD: input,
      });
    }
  };
